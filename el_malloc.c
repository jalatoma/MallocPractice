...
el_blockhead_t *el_get_header(el_blockfoot_t *foot) {
    // set size to foot size (follow outline of above)
    size_t size = foot->size;
    // since it is a lower address, use ptr minus bytes 
...
el_blockhead_t *el_block_below(el_blockhead_t *block) {
    el_blockhead_t *foot = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t)); // find foot for later use
    
    // check if the block is equal to the heap start and set to null if so
    if ((void *) block == (void*) el_ctl.heap_start) {
        return NULL;
    } 

    // find lower block and make suew it is in the heap and then return it
...
void el_add_block_front(el_blocklist_t *list, el_blockhead_t *block) {
    // as shown in the project spects, follow this format to add block to the front 
    block->prev = list->beg;
    block->next = list->beg->next;
    block->prev->next = block; // set the block to the front 
    block->next->prev = block;
    list->bytes += block->size + EL_BLOCK_OVERHEAD; // adjust bytes
    list->length += 1; // increase length
}
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block) {
    // also given in project specs, removing block
    block->prev->next = block->next; // remove the block from the list 
    block->next->prev = block->prev;
    list->bytes -= (block->size + EL_BLOCK_OVERHEAD); // adjust bytes
    list->length -= 1; // adjust length
}
...
el_blockhead_t *el_find_first_avail(size_t size) {
    // set the pointer
    el_blockhead_t *pointer = el_ctl.avail->beg;

    // get the length of available to iterate through
    int len = el_ctl.avail->length;
    // check through every pointer until there is a size big enough and return that pointer
    for(int i = 0; i < len; i++){
        pointer = pointer-> next;
        if (pointer->size >= (size + EL_BLOCK_OVERHEAD)){
            return pointer;
        }
    }
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size) {
    // returns NULL if not sufficient size
    if (block->size < (new_size + EL_BLOCK_OVERHEAD)){
        return NULL;
    }

    //get old block size and old foot
    size_t oldsize = block->size;
    el_blockfoot_t *footsplit = el_get_footer(block);
    // set block to new size
    block->size = new_size; 
    // get the new footer
    el_blockfoot_t *footer = el_get_footer(block);
    // set footer to new size
    footer->size = new_size;
    // get header and set the new head and foot sizes to their old minus new plus block overhead
    el_blockhead_t *headsplit = el_block_above(block);
    headsplit->size = oldsize - new_size - EL_BLOCK_OVERHEAD;
    footsplit->size = oldsize - new_size - EL_BLOCK_OVERHEAD;
...
void *el_malloc(size_t nbytes) {
    ...
    if (first == NULL){
        ...
        // otherwise, remove the block and split it 
        el_remove_block(el_ctl.avail, first);
        el_blockhead_t *newblock = el_split_block(first, nbytes);
        // set it's state to available since it is malloc'ed
        newblock->state = EL_AVAILABLE;
        // ad blocks to front as used and available for new blocks
        el_add_block_front(el_ctl.avail, newblock);
        el_add_block_front(el_ctl.used, first);
        // set the originial block state to used
        first->state = EL_USED;
        // return a pointer to usable space 
        return PTR_PLUS_BYTES(first, sizeof(el_blockhead_t));
    }
}
...
void el_merge_block_with_above(el_blockhead_t *lower) {
    // check the conditions as specified in the description, returns and does nothing if true 
    if(lower == NULL || lower->state != EL_AVAILABLE){
        return;
    }
    // get the higher or above block
    el_blockhead_t *higher = el_block_above(lower);

    // check the same conditions as the lower block with the hogher block, returns and does nothing if true 
    if(higher == NULL || higher->state != EL_AVAILABLE){
        return;
    }

    // otherwise, get the footer and remove both the higher and lower blocks
    el_blockfoot_t *foot = el_get_footer(higher);
    el_remove_block(el_ctl.avail, lower);
    el_remove_block(el_ctl.avail, higher);
    // reset the lower size 
    lower->size += higher->size + EL_BLOCK_OVERHEAD;
    // reset the foot size since blocks are merged
    foot->size = lower->size;
    // add lower block to the front 
...
void el_free(void *ptr) {
    // get the free block and the block below it 
    el_blockhead_t *free = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t));
    el_blockhead_t *below = el_block_below(free);
    // remove the block that needs to be freed
    el_remove_block(el_ctl.used, free);
    // set it's state to available
    free->state = EL_AVAILABLE;
    // add free block to the front
    el_add_block_front(el_ctl.avail, free);
    // merge the below and free block so that the free block is done being freed
